 ...
 * ## 根据分层的方式，使用动态规划
 ...
 那么，如何根据上一层每个点的高度、矩形左边界和矩形右边界，或者这一层的高度、矩形左边界和矩形右边界呢？对于节点[i,j]，它头顶上的节点是[i,j-1]，如果matrix[i][j]==0,则高度为0，否则，高度为matrix[i][j-1]+1。左边界，是取[i,j-1]计算得到的左边界和[i,j]
 最左侧的连续为1的节点的边界的靠右（较大）值，相反，右边界，是取[i,j-1]计算得到的右边界和[i,j]最右侧的连续为1的节点的边界的靠左（较小）值；
 注意，对于遇到`0` , 显然，面积是0，我们将它的高度设置为0就行，那么左边界和有边界呢，我们设置左边界为0，右边界为n，这样，下一层节点的左边界就直接由下一层节点决定，而不受本层的0的影响，右边界的原理也是如此。
 ...
 Largest Rectangle in Histogram问题通过栈的方式，计算最大矩形。本问题与之相似，只是，我们需要从第一层开始逐层往下计算。比如，
 我们先计算第一层matrix[i,0]的最大矩形，同时，我们还需要记录这一层每个节点的高度，然后，到每一层，我们都通过上一层的高度和这一层的节点值计
 算这一层的每一列的高度，然后同样通过栈的方式计算最大矩形，一直计算到matrix的最底层，获取最大矩形面积；
 */
