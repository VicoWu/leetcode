 ...
 * 这一题与[Count of Smaller Numbers After Self](https://leetcode.com/problems/count-of-smaller-numbers-after-self/description/)是几乎相同的问题，只是比较的标准不一样。
 ...
 # BST搜索方法
 我们为nums中的数字构建一个二叉搜索树，但是，树的节点中除了通过记载左右指针（left、right）和节点的数值val外，还需要通过一个变量cnt计算右子树中节点的数量，即，大于等于自己的节点的数量；因此。每次，我们往树种插入一个数字的时候，除了将其插入到指定的位置，而且，在遍历寻找指定位置的过程中，需要更新cnt的值，如下：
 ```
 ...
 ```
 可见，BIT树种保存的是元素的位置，而不是元素本身。遍历nums的过程中，遍历到元素A[i]，此时BIT中已经存放了位置在i之前的所有元素在nums中的排序以后的位置。因此，我们可以通过目前的BIT树知道`2*A[i]+1`在nums排序后的位置前面有多少个元素,从而计算这个数字A[i]对应的逆序数量，即，i之前但是大于`2*A[i]+1`的元素数量。举例：
 ```
 nums={4,1,2}
 排序后，copy={1,2,4}
