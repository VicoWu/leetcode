 ...
 * # 归并分治
 ...
 对于nums中的每一个数字a，通过在sortedCopy中二分查找得到a在sortedCopy中的位置，通过当前BIT树查找树中小于等于a的元素有几个（由于我们往BIT中插入数据的时候数值是1，因此，元素的值的和刚好等于元素的数量），就得到小于等于a且在nums中位于a右侧的元素个数，然后，我们把a所在的位置插入到BIT中。
 这样，我们在处理nums中的某个元素nums[i]的时候，BIT树中已经存放了nums中在i右侧的每一个元素在排序数组中的位置。因此，对于nums[i]这个元素，如果它再sortedArray中排序位置是k，那么，我们只需要查找BIT树里面在k左侧有多少个元素就行了。
