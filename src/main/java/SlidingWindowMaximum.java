 ...
 *
 ## 我之前想的方法：
 对于每一个window，保留这个window的最大值、左侧值和右侧值，这样，这个窗口在向右移动的过程中，只需要根据上一个窗口的最大值、左侧值和右侧值以及新窗口最新的右侧值(因为窗口向右移动)
 来获得新窗口的最大值、左侧值和右侧值，但是，这个无法获得，比如：1 [3  -1  -3] -4  3  6  7 此时窗口的最大值是最左侧的值，窗口向右移动一步，新的右边界为-4，
 比上一个窗口的最大值3要小，因此此时新窗口的最大值在内部而不是边界。
 ...
 * dequeue的队首始终保存的是当前window的最大元素
 ...
 最后的位置可能不足k个。想象着一个宽度为k的窗口逐渐移动，移动过程中，这个窗口或者不被切分，比如，窗口为[2,1,3,4]或者窗口为[6,3,8,9],或者窗口被切分，
 比如，窗口[3,4,6,3]，被切分为[3,4]和[6,3]，被切分的窗口，只需要获得两个子窗口[3,4]和[6,3]各自的最大值，就可以获得当前窗口[3,4,6,3]的最大值。
 [3,4]的最大值可以从左到右遍历原始数据，获取每一个切分的最大值序列：`2, 2, 3, 4 | 6, 6, 8, 9 | 10, 12, 56`，命名为A,[6,3]的最大值可以从右向左遍历，
 获得每一个切分的最大值序列：`4,4,4,4|9,9,9,9|56,56,56`，命名为B，这样，[3,4]的最大值就是B[2]=4（2是数字3所在的位置），[6,3]的最大值就是A[5]=6（5是数字3所在的位置），
 从而，[3,4,6,3]的最大值是6.
