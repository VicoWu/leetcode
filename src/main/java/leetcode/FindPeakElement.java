 ...
 * Question 162
 ...
 我们用反证法，假如在这种情况下，这个peek元素不存在，那么，[low,mid]以及[mid,hi]都是递增区间或者凹陷区间，比如(1,2,3,4)这种递增区间以及[4,3,2,3,4]这种凹陷区间，可以，如果[low,mid]以及[mid,hi]是递增区间或者凹陷区间，
 那么nums[mid]本身就成为了peek元素，与假设发生矛盾。
 ...
 比较nums[mid]和nums[mid+1]的大小，如果num[mid1] < num[mid2]，则num[mid1] 、num[mid2]以及num[n]形成了我们上面所证明的**拱形结构**，
 所以在[mid2,n]之间肯定能找到peek元素，否则，由于题目已经说明num中相邻的元素不相同，因此必有num[mid1] > num[mid2]，此时，nums[0],num[mid1],num[mid2]形成了**拱形结构**，所以在[0,mid1]之间肯定存在peek元素。这样形成了二分查找规则。
...
public class FindPeakElement {
    ...
    /**
     * 这一题的意思是，由于这个数组的两个边界都是无穷小，因此，其实，我们尽量找一个更大的元素，才有可能是一个peek
     * @param num
     * @return
     */
