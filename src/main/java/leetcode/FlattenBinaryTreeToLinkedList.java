...
public class FlattenBinaryTreeToLinkedList {
    ...
    private TreeNode flattenSubTree(TreeNode root) {
        if (root == null) {
            TreeNode leftEnd = flattenSubTree(root.left);
            TreeNode rightEnd = flattenSubTree(root.right);

            // 如果左子树不是空的，那么需要把左子树形成的链表的最后一个节点的right 指向右子树
            if (leftEnd != null) {
                leftEnd.right = root.right; //把右侧链表挂到左侧链表的最后一个节点的后面
                root.right = root.left; // 把左侧链表挂到跟节点的right节点
            }
            root.left = null;
            //返回组成的链表的最后一个节点
            if (rightEnd != null) {
