 ...
 * 然后再从尾端寻找另一个元素number，如果满足first 小于number，即将第first个元素与number元素对调，并将second元素之后（包括second）的所有元素颠倒排序，即求出下一个序列
 ...
 这一题首先要解决找到一个最靠右的元素A，这个元素大于其右侧的某个元素，这个寻找方法很简单，就是从尾端往前寻找两个相邻元素，前一个记为first，后一个记为second，并且满足first 小于 second
 找到这个元素以后，我们需要确定其右端的哪个元素B应该与这个元素交换，我们的方法是再从尾端寻找另一个元素number，如果满足first 小于number，即将第first个元素与number元素对调
 那么，对调以后，是不是就得到了我们要的结果呢？不是的，因为用B与A对调以后，B右侧的所有元素是从左到右降序排列，这时候必须对所有元素颠倒排序，才能得到下一个排列组合
 */
