 ...
 * [这里](https://leetcode.com/problems/combination-sum-iv/discuss/85036)非常详细地讲解了这一题的解答思路
 ...
 # 从上而下的递归方法
 假如nums=[1,2,3]，target=4，对于nums中的任何一个数字，比如1，有target-1=3，因此，我们需要从nums递归检索和为3的组合。因为题目中[1,2,1]，[1,1,2]和[2,1,1]是不同的组合，并且允许一个数字使用多次，因此，对剩余的和4-1=3，我们仍然在nums中查找，不会排除任何数字。
...
public class CombinationSumIV {
    ...
    public int combinationSum4_1(int[] nums, int target) {
        ...
        for (int i = 1; i < result.length; i++) {
            for (int j = 0; j < nums.length; j++) { //对于nums[]
                if (i >= nums[j])
